...
class QLearner:
    ...
    def set_go(self, go):
        ...
        self.min_max_width = MIN_MAX_WIDTH - len(self.go.move_list)
        self.min_max_depth = MIN_MAX_DEPTH + int(BOARD_SIZE - len(self.go.move_list) ** 0.5)
    ...
    def max_value(self, last_board, cur_board, alpha, beta, step):
        ...
        if step > self.min_max_depth:
            # move, Q = self.find_max_by_Q(move_list, self.Q(go_test))
            return "move", self.board_value(cur_board)
        ...
        v = -np.inf
        max_action = ()
        for move_i in range(max(len(ordered_move_list), MIN_MAX_WIDTH)):
            move = ordered_move_list[move_i]
    ...
    def learn(self, state_action_list):
        ...
        for state_action_state in state_action_list:
            ...
            if q_t == "NOVALUE":
                q_t = np.zeros((BOARD_SIZE,BOARD_SIZE))

            if q_t1 == "NOVALUE":
                q_t1 = np.zeros((BOARD_SIZE,BOARD_SIZE))
